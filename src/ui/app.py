...
from src.ui.input_dialog import InputDialog, ShiftTimeDialog
from src.ui.tabs import OverviewTab, DetailsTab, PathsTab, SettingsTab

from src.ui.plan_tab import EnhancedPlanCalculator, PlanTab
...
class DataProcessingThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            try:
                results = loop.run_until_complete(processor.process_incoming_data())
                error_summary = ""
                # Unpack the results
                if isinstance(results, tuple) and len(results) == 2:
                    processed_data, failed_data = results
                    
                    # If there are failed requests, emit the error but don't stop
                    if failed_data:
                        # Format the error message
                        error_messages = []
                        for name, details in failed_data.items():
                            error_messages.append(
                                f"{name}: Status {details['status']}\n{details.get('errror', 'Unknown error')}"
                            )
                        error_summary = "Some requests failed:\n" + "\n".join(error_messages)
                    else:
                        error_summary = ""

                    self.error.emit(error_summary)
                    
                    # Still emit the processed data to continue the application
                    self.finished.emit(processed_data)
                else:
                    # Handle case where only processed_data is returned
...
class SDCPickAssistApp(qtw.QMainWindow):
    def __init__(self):
        ...
        self.plan_calculator = EnhancedPlanCalculator.get_instance()
        ...
        self.is_pad_set = False # Pad Set Flag
        ...
        self.tab_plan.refresh_overview.connect(
            self.update_tabs
        )
    ...
    def on_processing_complete(self, results):
        ...
        SiteBuilder.get_instance().get_site_info(new=True)
    ...
    def update_tabs(self):
        self.tab_plan.delayed_update(edit=False)
    ...
    def on_processing_error(self, error_msg):
        ...
        self.go_button.setEnabled(True)
        self.go_button.setText("Error")
        self.go_button.setToolTip(error_msg)
        self.go_button.setStyleSheet("""
            QPushButton {
        ...
        """)
        self.show_timed_warning(f"Error during refresh:\n{error_msg}")
    ...
    def create_top_frame(self):
        ...
        self.link_button.setToolTip("Click to change shift times")
        self.link_button.clicked.connect(lambda: self.check_shift_times(force=True))

        self.frame0.layout().addWidget(self.link_button, 0, 2, alignment=qtc.Qt.AlignCenter)
    ...
    def on_site_changed(self, new_site):
        ...
        if success:
            ...
            EnhancedPlanCalculator.reset_instance()
            ...
            self.site_info = self.site_builder.get_site_info(new=True)
            self.pick_areas = self.site_info['pick_areas']

            # Drop loaded Shift plan
            self.plan_calculator = self.plan_calculator.get_instance(new=True)
    ...
    def handle_go_button_click(self, first_run=False):
        ...
        self.pad_state = self.tab_settings.pad_state
        ...
        # Check if pad is set
        if not self.is_pad_set and self.pad_state:
            logger.info('Pad not set')


        # Check if shift times are valid
        if not self.check_shift_times():
            logger.info('Invalid shift times not updated')
            self.show_timed_warning("Invalid shift times. Please update.")
            return
    ...
    def check_shift_times(self, force=False):
        remaining_time = self.shift_info['hours_remaining'].total_seconds() / 3600
        if remaining_time < 0 or force:
            ...
            # Create and show shift time update dialog
            shift_dialog = ShiftTimeDialog()
            shift_dialog.show_input_dialog()
            
            # If dialog was successful, update the time manager with new values
            if shift_dialog.dialog_result:
                start_hour = shift_dialog.start_hour
                end_hour = shift_dialog.end_hour
                ...
                if success:
                    ...
                    self.update_shift_progress_bar()
                    print("Shift time updated successfully")
                    logger.info("Shift time updated successfully")


                    return True
                else:
                    logger.error("Failed to update shift times")
                    return False
            else:
                logger.info("Shift time update cancelled")
                return False
        else:
            self.time_manager.update_shift()
            self.shift_info = TimeManager.get_instance().get_shift_info()
            ...
            print(f'Shift time remaining: {remaining_time} hours')
            logger.info(f'Shift time remaining: {remaining_time} hours')
