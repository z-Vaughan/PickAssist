...
class OverviewTab(qtw.QWidget):
    ...
    def gather_data(self):
        ...
        if self.data['Rodeo'] is not None:
            ...
            self.hov_picks_remaining = int(self.data['Rodeo']['cpt_summary'][0,3]) or 0
            self.time_remaining = float(self.data['Rodeo']['cpt_summary'][0,4]) or 0
            ...
            if self.data['Rodeo']['cpt_summary'].height > 1:
                ...
                self.hov_picks_remaining2 = int(self.data['Rodeo']['cpt_summary'][1,3]) or 0
            else:
                self.next_cpt = 'None'
        ...
        if self.plan_data is not None:
            self.target_volume = self.plan_data['plan']['volume']
            self.target_rate = self.plan_data['plan']['rate']
            self.target_hours = self.plan_data['plan']['hours']
            self.plan_hc = self.plan_data['plan']['hc']
    ...
    def populate_overview_ui(self):
        ...
        if self.plan_data is not None:
            self.percent_to_plan = {
                ...
                'headcount': calculate_percent_to_plan(self.active_hc, self.plan_hc),
        ...
        picks_density_label2 = qtw.QLabel(
            f"Picks Remaining: {picks_remaining} (HOV: {hov_picks_remaining})\nPick Density: {pick_density:.2f}"
        )
                    ...
                    if col_name == 'Current HC':
                        ...
                        if target_hc > 0:
                            ...
                            if deviation < -0.25:  # More than 25% understaffed
                                ...
                                item.setToolTip(f"Understaffed by {abs(deviation)*100:.1f}%")
                            elif deviation > 0.25:  # More than 25% overstaffed
                                item.setBackground(qtg.QColor('#ffcccc'))
                                item.setToolTip(f"Overstaffed by {deviation*100:.1f}%")
                            else:
...
class PathsTab(qtw.QWidget):
                ...
                for path in unique_paths:
                    try:
                        ...
                        try:
                            ...
                            logger.error(f"Error creating missing area dataframe: {str(e)}", exc_info=True)
                                            ...
                                            for group_idx, (group_rows, color) in enumerate(self._group_pick_areas(palatable_data)):
                                                ...
                                                if row in group_rows:
                                                    ...
                                                    try:
                                                        ...
                                                        group_picks = sum(float(r['Picks']) for r in group_rows)
                                                    ...
                                                    for group_row in group_rows:
                                                        try:
                                                            picks = float(group_row['Picks'])
                                                            current_cph = float(group_row['CPH']) or 0
                                                            historical_cph = float(group_row['7-Day CPH']) or 0
                                                            
                                                            logger.debug(f"Row data - Picks: {picks}, CPH: {current_cph}, Historical CPH: {historical_cph}")
                                                            
                                                            effective_cph = current_cph if current_cph > 0 else historical_cph
                                                            if effective_cph > 0:
                    ...
                    try:
                        ...
                        if total_hc > 0 and current_cph > 0:
                            try:
                                time_to_exhaust = total_picks / (total_hc * current_cph)
    ...
    def _group_pick_areas(self, palatable_data):
        ...
        group_colors = [
            qtg.QColor(255, 200, 200, 255, 80),  # Darker pink/red
    ...
    def show_missing_areas_dialog(self, cpt, path):
        ...
        try:
            ...
            logger.error(f"Error showing missing areas dialog: {str(e)}", exc_info=True)
...
class PlanTab(qtw.QWidget):
    def __init__(self, parent=None):
        ...
        self.calculator = PlanCalculator()

        self.calculator.plan_saved.connect(self.gather_plan)

        self.gather_plan()



    def gather_plan(self, update=False):
        self.gather_plan_data()
        self.populate_plan_ui(update=update)
        


    def gather_plan_data(self):
        """
        Gathers plan data from the input fields and returns a dictionary.
        """
        self.shift_info = TimeManager.get_instance().get_shift_info()
        self.site_info = SiteBuilder.get_instance().get_site_info()

        self.hours_remaining = self.shift_info['hours_remaining'].seconds / 3600
        self.shift_hours = self.shift_info['total_hours'].seconds / 3600
        self.hours_passed = self.shift_info['elapsed_time'].seconds / 3600
        self.recent_plan = self.site_info['plan_data']
        self.site_code = self.site_info['site_code']
        ...
        self.data = DataProcessor.get_instance().get_results()
        if hasattr(DataProcessor,'processed_data'):
            self.hours = self.data['LPI']['combined']['combined_hrs']
        else:
            self.hours = 0



    def populate_plan_ui(self, update=False):
            """
            Populates the 'Plan' tab with either the most recent plan data in read-only mode
            or input fields if no recent plan exists.
            """
            # Clear existing widgets
            for widget in self.findChildren(qtw.QWidget):
                widget.deleteLater()
            ...
            # Create and set new grid layout
            grid_layout = qtw.QGridLayout()
            self.setLayout(grid_layout)
            
            # Configure grid
            grid_layout.setColumnStretch(2, 1)
            for i in range(5):  # Added one more row for the overwrite button
                grid_layout.setRowStretch(i, 1)

            
            if self.recent_plan:
                self.display_readonly_plan(self.recent_plan)
            else:
                self.display_plan_input_fields(update=update)


    def display_readonly_plan(self, plan_data):
        """
        Displays the plan data in read-only mode with an overwrite button.
        """
        ...
        # Create main layout as QHBoxLayout for side-by-side columns
        main_layout = qtw.QHBoxLayout()
        self.setLayout(main_layout)

        # Create left column for fields
        left_column = qtw.QWidget()
        left_layout = qtw.QVBoxLayout(left_column)
        
        # Create plan details widget
        details_widget = qtw.QWidget()
        details_layout = qtw.QGridLayout(details_widget)
        details_widget.setSizePolicy(qtw.QSizePolicy.Preferred, qtw.QSizePolicy.Fixed)

        header_label = qtw.QLabel('Plan Details')
        header_label.setFont(qtg.QFont("Helvetica", 18))
        header_label.setAlignment(qtc.Qt.AlignCenter)
        left_layout.addWidget(header_label, alignment=qtc.Qt.AlignCenter)

        # Add plan details
        fields = {
            'rate': {'label': 'Rate:', 'row': 0, 'value': plan_data['plan']['rate']},
            'volume': {'label': 'Volume:', 'row': 1, 'value': plan_data['plan']['volume']},
            'hours': {'label': 'Labor Hours:', 'row': 2, 'value': plan_data['plan']['hours']},
            'is RT?' : {'label': 'Is RT?', 'row': 3, 'value': plan_data['shift']['is RT?']}
        }

        for field_name, config in fields.items():
            label = qtw.QLabel(config['label'])
            label.setFont(qtg.QFont("Helvetica", 14))
            details_layout.addWidget(label, config['row'], 0, alignment=qtc.Qt.AlignRight)

            value_label = qtw.QLabel(str(config['value']))
            value_label.setFont(qtg.QFont("Helvetica", 14))
            details_layout.addWidget(value_label, config['row'], 1, alignment=qtc.Qt.AlignLeft)

        left_layout.addWidget(details_widget, alignment=qtc.Qt.AlignCenter)
        left_layout.addStretch()

        # Add submission info to left column
        info_widget = qtw.QWidget()
        info_widget.setSizePolicy(qtw.QSizePolicy.Preferred, qtw.QSizePolicy.Fixed)
        info_layout = qtw.QVBoxLayout(info_widget)

        # Format and add submission info
        timestamp = dt.fromisoformat(plan_data['timestamp'])
        formatted_timestamp = timestamp.strftime("%Y-%m-%d %H:%M:%S")

        submission_info = qtw.QLabel(
            f"Submitted by: {plan_data['submitted_by']}\n"
            f"Timestamp: {formatted_timestamp}"
        )
        italic_font = qtg.QFont("Helvetica", 10)
        italic_font.setItalic(True)
        submission_info.setFont(italic_font)
        submission_info.setAlignment(qtc.Qt.AlignCenter)
        info_layout.addWidget(submission_info)

        left_layout.addWidget(info_widget, alignment=qtc.Qt.AlignCenter)

        # Create right column for message
        right_column = qtw.QWidget()
        right_layout = qtw.QVBoxLayout(right_column)

        message_label = qtw.QLabel('Message:')
        message_label.setFont(qtg.QFont("Helvetica", 14))
        right_layout.addWidget(message_label, alignment=qtc.Qt.AlignCenter)

        # Create scroll area for message text
        scroll_area = qtw.QScrollArea()
        scroll_area.setWidgetResizable(True)
        scroll_area.setFrameShape(qtw.QFrame.NoFrame)
        
        message_text = qtw.QLabel(plan_data.get('message', 'N/A'))
        message_text.setFont(qtg.QFont("Helvetica", 14))
        message_text.setWordWrap(True)
        message_text.setAlignment(qtc.Qt.AlignCenter)
        message_text.setSizePolicy(qtw.QSizePolicy.Preferred, qtw.QSizePolicy.Minimum)
        
        scroll_area.setWidget(message_text)
        right_layout.addWidget(scroll_area)


        # Add overwrite button at the bottom
        button_container = qtw.QWidget()
        button_layout = qtw.QHBoxLayout(button_container)
        
        if self.data:
            self.overwrite_button = qtw.QPushButton("Overwrite Plan")   
            self.overwrite_button.setEnabled(True)
            self.overwrite_button.clicked.connect(self.confirm_overwrite)
        else:
            self.overwrite_button = qtw.QPushButton("Run PickAssist to Overwrite")   
        ...
        main_layout.setStretch(1, 1)  # Right column
    ...
    def confirm_overwrite(self):
        ...
        if reply == qtw.QMessageBox.Yes:
            ...
            if recent_network_files_exist:
                ...
                if reply == qtw.QMessageBox.Yes:
                    self.display_plan_input_fields(overwrite=True)
    ...
    def display_plan_input_fields(self,update=False, overwrite=False):
        """
        Displays the input fields for creating a new plan.
        """
        # First clear the tab completely
        if self.layout() is not None:
            while self.layout().count():
                item = self.layout().takeAt(0)
                if item.widget():
        ...
        # Create main layout as VBox
        main_layout = qtw.QVBoxLayout()
        self.setLayout(main_layout)
        
        # Create container for input fields
        input_container = qtw.QWidget()
        input_layout = qtw.QHBoxLayout(input_container)
        
        # Left column container for rate, volume, hours
        left_column = qtw.QWidget()
        left_layout = qtw.QFormLayout(left_column)
        left_layout.setContentsMargins(20, 20, 20, 20)  # Add some padding
        left_layout.setSpacing(10)  # Space between rows
        

        header_label = qtw.QLabel('Target Metrics')
        header_label.setFont(qtg.QFont("Helvetica", 14))
        header_label.setAlignment(qtc.Qt.AlignCenter)
        left_layout.addWidget(header_label)
        # Create and add left column fields
        fields = {
            'rate': 'Rate:',
            'volume': 'Volume:',
            'hours': 'Labor Hours:'
        ...
        for field_name, label_text in fields.items():
            # Create label with consistent font
            label = qtw.QLabel(label_text)
            label.setFont(qtg.QFont("Helvetica", 14))
            
            # Create line edit
            entry = qtw.QLineEdit()
            entry.setFont(qtg.QFont("Helvetica", 14))
            setattr(self, f"target_{field_name}_entry", entry)
            
            # Add to layout
            left_layout.addRow(label, entry)
        
        # Add checkbox for is_rt below the main input fields
        rt_checkbox = qtw.QCheckBox("Check for RT Shift")
        rt_checkbox.setFont(qtg.QFont("Helvetica", 14))
        rt_checkbox.setChecked(getattr(self, 'is_rt', False))  # Set initial state based on self.is_rt
        rt_checkbox.stateChanged.connect(lambda state: setattr(self, 'is_rt', bool(state)))  # Update self.is_rt when changed
        left_layout.addRow("", rt_checkbox)  # Empty label for alignment

        # Right column container for message
        right_column = qtw.QWidget()
        right_layout = qtw.QVBoxLayout(right_column)
        right_layout.setContentsMargins(20, 20, 20, 20)  # Add some padding
        
        # Create message label
        message_label = qtw.QLabel('Message (optional)')
        message_label.setFont(qtg.QFont("Helvetica", 14))
        message_label.setAlignment(qtc.Qt.AlignCenter)
        right_layout.addWidget(message_label)
        
        # Create message entry
        self.target_message_entry = qtw.QTextEdit()
        self.target_message_entry.setFont(qtg.QFont("Helvetica", 14))
        self.target_message_entry.setMinimumHeight(100)  # Set minimum height
        self.target_message_entry.setMaximumHeight(200)  # Optional: set maximum height
        
        # Add some styling
        self.target_message_entry.setStyleSheet("""
            QTextEdit {
        ...
        self.target_message_entry.setPlaceholderText("Enter message here...")
        # Add vertical scroll bar if text exceeds the box height
        self.target_message_entry.setVerticalScrollBarPolicy(qtc.Qt.ScrollBarAsNeeded)
        # Set word wrap
        self.target_message_entry.setLineWrapMode(qtw.QTextEdit.WidgetWidth)
        
        right_layout.addWidget(self.target_message_entry)
        
        # Add stretch to push message entry to top
        right_layout.addStretch(1)
        
        # Add columns to input container
        input_layout.addWidget(left_column)
        input_layout.addWidget(right_column)  # Give right column more stretch
        
        # Add input container to main layout
        main_layout.addWidget(input_container)
        
        # Add stretch to push calculate button to bottom
        main_layout.addItem(qtw.QSpacerItem(0, 20))
        ...
        if overwrite and self.data:
            
            self.calculate_button = qtw.QPushButton("Overwrite Plan")   
            self.calculate_button.setEnabled(True)
            self.calculate_button.clicked.connect(
                lambda: self.calculator.save_plan(
                    int(self.target_volume_entry.text()),
                    float(self.target_rate_entry.text()),
                    float(self.target_hours_entry.text()),
                    self.is_rt,
                    self.target_message_entry.toPlainText()
                )
            )


        elif update and self.data:
                    self.calculate_button = qtw.QPushButton("Submit Plan")
                    self.calculate_button.setEnabled(True)
                    self.calculate_button.clicked.connect(
                        lambda: self.calculator.save_plan(
                        int(self.target_volume_entry.text()),
            ...
            self.calculate_button = qtw.QPushButton("Run PickAssist To Calculate")
            self.calculate_button.setEnabled(False)

        # Style calculate button
        self.calculate_button.setFont(qtg.QFont("Helvetica", 14))
        self.calculate_button.setFixedWidth(300)  # Set fixed width for button
        
        # Add button to main layout centered
        main_layout.addWidget(self.calculate_button, alignment=qtc.Qt.AlignCenter)
              
class PlanCalculator(QObject):
    plan_saved = Signal()
    ...
    def __init__(self):
        ...
        try:
            ...
            site_info = self.site_builder.get_site_info()
            self.plan_data = site_info['plan_data']
            self.site_code = site_info['site_code']
            self.shift_info = TimeManager.get_instance().get_shift_info()
            self.hours_passed = self.shift_info['elapsed_time'].seconds / 3600
            self.shift_hours = self.shift_info['total_hours'].seconds / 3600
            self.hours_remaining = self.shift_info['hours_remaining'].seconds / 3600
            ...
            logger.error(f"Error initializing PlanCalculator: {str(e)}")
            raise
    

    def save_plan(self, target_volume, target_rate, target_hours, is_rt, opt_msg=''):
        """
        Saves the plan data to a file and returns the plan data.
        Attempts to save to network location first, falls back to local if network is inaccessible.
        """

        hours_remaining = self.hours_remaining
        shift_hours = self.shift_hours
        hours_passed = self.hours_passed
        recent_plan = self.plan_data

        hours = self.data['LPI']['combined']['combined_hrs']
            

        logger.info('=== Starting save_plan_file ===\n')
        logger.info(f'Input parameters - Volume: {target_volume}, Rate: {target_rate}, '
                    f'Hours: {target_hours}, Message: {opt_msg}')
        
        
        projected_hours = target_volume / target_rate
        # Shift ownership = Actual Shift +1hr, so we take half an hour from the hours remaining
        plan_hc = math.ceil((projected_hours - hours) / (hours_remaining - .5))
        ...
        try:
            ...
            plan_data = {
                ...
                "shift": {
                    ...
                    "time_remaining": hours_remaining,
                    "time_passed": hours_passed,
                    "is RT?": is_rt
                },
                "message": opt_msg
            }
            ...
            try:
                ...
                resource_path = find_resource(f"{self.site_code}_shift_plan.json")
...
class SettingsTab(qtw.QWidget):
    ...
    def populate_settings_tab(self):
        ...
        self.display_layout.addItem(qtw.QSpacerItem(0, 10))
